...
def gillespie (v_old, t_old, rates, mu, tau, K) :
    ...
    # rates
    if K : # if the growth is density-dependent, multiply intrinsic growth rates by 1-pop_size/K
        pop_size = sum(v_old)
        rate_log = np.copy(rates)
        rate_log[0] = [rates[0][i]*max(0,1-pop_size/K) for i in range(5)]
    ...
    death_rates2 = [rates[1][i]*v_old[i] for i in range(5)]
...
def sim_pop (n, nb_cycles, alpha, beta, delta, rates, tau = 0.05, gamma = None) :
    """
    ...
    Arguments:
        ...
        rates: list of intrinsic birth and death rates of the 5 subpopulations
        tau: tau-leaping parameter
        gamma: parameter for carrying capacity (set to None to make a
        simulation without density-dependence)
    Returns:
        t: list of event times (t[i] is the time at step i)
    ...
    tn = np.log(n)*(alpha-beta)/r00 # duration of the growth phase
    Dn = n**(beta-alpha) # dilution factor
    ...
    # simulate nb_cycles
    while c < nb_cycles and sum(start) > 0 :
        # simulation of growthg phase
        if gamma :
            sim = sim_cycle(tn, start, rates, mu, tau, K = n**gamma)
        else :
    ...
    # attribute scenario number according to observed evolutionary paths
...
def plot_sim (n, nb_cycles, alpha, beta, delta, rates, gamma = None, sep_double = True, leg_loc = "lower left") :
    """
    ...
    """
    sim = sim_pop(n, nb_cycles, alpha, beta, delta, rates, gamma = gamma)
...
def many_sim_pop (n_sim, n, nb_cycles, delta, rates, gamma = None) :
    """
...
def make_fig_paths (A, B, S, predictions = False, rates = None, delta = None) :
    """
...
def make_fig_double (A, B, C, predictions = False, rates = None, delta = None) :
    """
